...
TEST(ObjectDupe, DupeArray)
{
    olib_object_t* original = olib_object_new(OLIB_OBJECT_TYPE_ARRAY);

    for (int i = 0; i < 3; i++) {
        olib_object_t* val = olib_object_new(OLIB_OBJECT_TYPE_INT);
        olib_object_set_int(val, i * 10);
        olib_object_array_push(original, val);
    }

    olib_object_t* copy = olib_object_dupe(original);
    ASSERT_NE(copy, nullptr);
    EXPECT_EQ(olib_object_array_size(copy), 3u);

    for (int i = 0; i < 3; i++) {
        EXPECT_EQ(olib_object_get_int(olib_object_array_get(copy, i)), i * 10);
    }

    // Modifying copy shouldn't affect original (deep copy)
    olib_object_set_int(olib_object_array_get(copy, 0), 999);
    EXPECT_EQ(olib_object_get_int(olib_object_array_get(original, 0)), 0);
...
{
    // Create nested structure: struct containing array containing struct
    ...
    olib_object_array_push(arr, inner);
    olib_object_struct_add(root, "array", arr);

    olib_object_t* copy = olib_object_dupe(root);
    ASSERT_NE(copy, nullptr);

    olib_object_t* copy_arr = olib_object_struct_get(copy, "array");
    ASSERT_NE(copy_arr, nullptr);
    olib_object_t* copy_inner = olib_object_array_get(copy_arr, 0);
