...
TEST(EdgeCases, StringWithNullCharacter)
{
    ...
    EXPECT_TRUE(olib_object_set_string(obj, "before\x01\x02\x03" "after"));
    EXPECT_STREQ(olib_object_get_string(obj), "before\x01\x02\x03" "after");
...
TEST(EdgeCases, SingleElementMatrix)
{
    size_t dims[] = {1};
    olib_object_t* mat = olib_object_matrix_new(1, dims);
    ASSERT_NE(mat, nullptr);

    EXPECT_EQ(olib_object_matrix_total_size(mat), 1u);

    size_t idx[] = {0};
    EXPECT_TRUE(olib_object_matrix_set(mat, idx, 42.0));
    EXPECT_DOUBLE_EQ(olib_object_matrix_get(mat, idx), 42.0);

    olib_object_free(mat);
}

TEST(EdgeCases, LargeMatrix)
{
    size_t dims[] = {100, 100};
    olib_object_t* mat = olib_object_matrix_new(2, dims);
    ASSERT_NE(mat, nullptr);

    EXPECT_EQ(olib_object_matrix_total_size(mat), 10000u);

    EXPECT_TRUE(olib_object_matrix_fill(mat, 1.5));

    size_t idx[] = {50, 50};
    EXPECT_DOUBLE_EQ(olib_object_matrix_get(mat, idx), 1.5);
